...
The `home-routes` handler in the `todo-app.routes.home` namespace
...
The `home-page` function will in turn call the `todo-app.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `todo-app.handler` namespace:
...
Request middleware functions are located under the `todo_app.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `todo-app.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `todo-app.db.core` in some other namespace.
